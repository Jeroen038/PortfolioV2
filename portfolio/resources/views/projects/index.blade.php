    ...
    <main class="ml-16 min-h-screen flex">
        ...
        <div class="flex items-center py-20 w-5/6 flex-col px-20">
            @forelse ($projects as $project)
                ...
                @empty
                <div class="flex flex-col items-center justify-center text-gray-400 mt-10 h-[400px]">
                    <img src="{{ asset('storage/no-projects.png') }}" alt="Geen projecten gevonden"
                         class="w-64 h-auto opacity-80">

                    <h3 class="text-xl font-semibold mt-5">Geen projecten gevonden!</h3>
                    <p class="text-gray-500 text-sm text-center max-w-sm">Probeer een andere combinatie van filters of voeg nieuwe projecten toe.</p>

                    @if (!empty($selectedTechs))
                        <a href="{{ route('projects.index') }}"
                           class="mt-6 bg-red-600 px-5 py-2 rounded-lg hover:bg-red-700 transition text-white">
                            Reset filters
                        </a>
                    @endif
                </div>
